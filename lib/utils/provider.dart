...
class Provider {
  static const String _urlCookie =
  ...
  static Future<Map> _isSceltaCarriera(
    ...
    var lengthScelta = tableScelta.children.length;
  ...
  static Future<Map> getAccess(var basicAuth64, var username) async {
    var mapInfo = {};
    //Ottengo il cookie di sessione per la request
    var getCookie;
    try {
      getCookie = await http.head(_urlCookie);
    } catch (e) {
      mapInfo['success'] = false;
      mapInfo['error'] = e;
      return mapInfo;
    ...
    var requestUrl = _urlCookie + ';jsessionid=' + jsessionId;
    ...
    //Scraping home page
    var documentHome = parser.parse(response.body);
  ...
  static String _getNomeInfo(var n) {
    switch (n) {
      case 1:
        return 'tipo_corso';
      case 3:
  ...
  static Future<Map> getInfoAppello(var urlInfo) async {
    var prefs = await SharedPreferences.getInstance();
    var basicAuth64 = prefs.getString('auth64Cred');
    ...
    var lunghezzaHidden = tabellaHidden.children.length;
  ...
  static Future<Map> prenotaAppello(var infoAppello) async {
    ...
    var lunghezzaHidden = infoAppello['lunghezzaHidden'];
